MODULE_NAME='Epihan_Module'    (dev dv,
				dev tp)
...
DEFINE_CALL 'FB'
{
    if (Status='') SEND_STRING dv,"'STATUS',$0D"
    if (find_string(Buf_Ep,"$0A",1))
    {
	Buf_Min = REMOVE_STRING(Buf_Ep,"$0A",1)
	WaitTime = 0
	IF  (FIND_STRING(Buf_Min,"'Rectime '",1))
	{
	    remove_string(Buf_Min,"'Rectime '",1)
	    RecTime = atoi(Buf_Min)
	    send_command tp,"'^TXT-2,0,',itoa(RecTime/60),'.',itoa(RecTime%60)"
	}
	Else
	if (FIND_STRING(Buf_Min,"'Status '",1))
	{
	    remove_string(Buf_Min,"'Status '",1)
